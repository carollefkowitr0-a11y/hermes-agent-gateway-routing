...
def default_spool_db_path(profile: str = DEFAULT_PROFILE) -> Path:
    ...
    return get_default_hermes_root() / "profiles" / profile / "gateway_spool.db"
...
def run_worker(
    *,
    ...
    """Run one local dry-run spool worker iteration and return a safe summary."""

    profile = (profile or "").strip()
    resolved_db = Path(spool_db) if spool_db is not None else None
    if not once:
        return _refusal_summary(profile=profile or DEFAULT_PROFILE, spool_db=resolved_db, reason="only_once_supported")
    if profile != DEFAULT_PROFILE:
        return _refusal_summary(profile=profile, spool_db=resolved_db, reason="non_naval_profile_refused")
    ...
    controller = WakeController(
        ...
        consumer=consumer,
        allow_non_naval=False,
    )
    result = controller.run_once()
    return _safe_worker_summary(profile=profile, spool_db=db_path, result=result, dry_run=dry_run)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run one local dry-run gateway spool worker iteration.")
    parser.add_argument("--profile", default=DEFAULT_PROFILE, help="Target profile; only 'naval' is accepted.")
